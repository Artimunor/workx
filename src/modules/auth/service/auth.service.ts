...
import { v4 } from 'uuid';
import { AuthHelper } from '../util/auth.util';
...
export class AuthService {
  ...
  public async userRegister(registerInput: RegisterInput): Promise<UserModel> {
    ...
    return {
      token: this.signToken({ userId: userCreated.id }),
  ...
  public async userLogin(loginInput: LoginInput): Promise<UserModel> {
    ...
    return {
      token: this.signToken({ userId: user.id }),
      email: user.email,
  ...
  public async userLogout(user: UserEntity, token: string): Promise<boolean> {
  ...
  public signToken(jwtInput: JwtInput): string {
    return this.jwt.sign(jwtInput);
  }
